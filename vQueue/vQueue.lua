...
glGroups = groups
SselectedQuery =selectedQuery
SselectedCat=selectedCat
...
BLeaders = vQueueDB.bannedLeaders
...
--[=[		categories[L["Miscellaneous"]] =
		...
		{
			expanded = false,
			BZ["Hellfire Ramparts"]..":ramp",
			BZ["The Blood Furnace"]..":bf",
			BZ["The Slave Pens"]..":sp",
			BZ["The Underbog"]..":ub",
			BZ["Mana-Tombs"]..":mana",
			BZ["Auchenai Crypts"]..":ac",
			BZ["Sethekk Halls"]..":seth",
			BZ["Old Hillsbrad Foothills"]..":cot1",
			BZ["The Mechanar"]..":mech",
			BZ["The Black Morass"]..":cot2",
			BZ["Shadow Labyrinth"]..":sl",
			BZ["The Steamvault"]..":sv",
			BZ["The Shattered Halls"]..":sh",
			BZ["The Arcatraz"]..":arca",
			BZ["The Botanica"]..":bota",
...
--[=[		categories[L["Quest Groups"]] =
		...
		vQueueFrame:SetScript("OnShow", function()
			...
			this:SetScale(1/UIParent:GetScale())
		end)
		...
		vQueueFrame.catList:SetScript("OnMouseWheel", function(self, delta)
...
-- Sven Button Ende
		...
		vQueue_createCategories(L["Dungeons TBC"])
	...
	for k, v in pairs(categories[textKey]) do
		...
		if type(args[1]) == "string" then
			...
			dropedItemFrame:SetScript("OnMouseDown", function()
				...
				vQueueFrame.hostTitleFindSize:Show()
				vQueueFrame.hostTitleFindRoles:Show()
				if not vQueueDB.isHost then realHostedCategory = trim(split(this:GetText(), "%[")[1]) end
				local args = {}
				for k, v in pairs(categories) do
					for i, item in pairs(v) do
						if type(item) == "string" then
				...
				scrollbar:SetValue(1)
				scrollbar:SetMinMaxValues(1, tablelength(groups[selectedQuery])>10 and tablelength(groups[selectedQuery])-10 or 1)
			end)
...
function vQueue_ShowGroups(category, prevCategory)
	...
	if category ~= "" then
		...
		scrollbar:SetMinMaxValues(1, tablelength(groups[category])>10 and tablelength(groups[category])-10 or 1)
	end
	vQueue_UpdateHostScroll(scrollbar:GetValue())
end
